...
const defaultItems: ContactInfoItem[] = [
  ...
  },
  {
    ...
    content: (
      <>
        <p className="text-neutral-400 mb-2">Main Office:</p>
        <a href="tel:+1234567890" className="text-secondary hover:text-secondary-dark">
          +1 (234) 567-890
        </a>
      </>
    )
...
const ContactInfo = ({ items = defaultItems }: ContactInfoProps) => {
  return (
    <div className="space-y-6">
      {items.map((item, index) => (
        <div className="flex items-start" key={index}>
          ...
          <div>
            <h4 className="font-heading font-bold text-lg text-primary mb-1">{item.title}</h4>
            {item.content}
          </div>
        </div>
      ))}
      ...
      <div className="pt-6 border-t border-neutral-200">
        <h4 className="font-heading font-bold text-lg text-primary mb-4">Follow Us</h4>
        <div className="flex space-x-4">
          <a 
            href="#" 
            className="w-10 h-10 bg-primary hover:bg-primary-dark rounded-full flex items-center justify-center text-white transition-colors duration-200"
            aria-label="Facebook"
          >
